...
def test_generate_drawers_predicate_form_without_entity_and_domain(
    ...
    with rule_mode():
        query = infer(
            Drawer(
                handle=a(handle := Handle(world=world)),
...
def test_rule_tree_with_a_refinement(doors_and_drawers_world):
    ...
    with symbolic_mode():
        query = an(
            entity(
                drawers_and_doors := let(type_=View),
...
def test_rule_tree_with_multiple_alternatives_better_rule_tree_optimized(
    ...
    with symbolic_mode():
        query = infer(views := let(type_=View), HasType(fixed_connection.child, Handle))
...
def test_rule_tree_with_multiple_alternatives_predicate_form_too_much_joins(
    doors_and_drawers_world,
):
    world = doors_and_drawers_world
    with symbolic_mode():
...
def test_rule_tree_with_multiple_alternatives_predicate_form_better_rule_tree(
    doors_and_drawers_world,
):
    world = doors_and_drawers_world
    with symbolic_mode():
...
def test_rule_tree_with_multiple_alternatives_predicate_form_better_rule_tree_optimized(
    doors_and_drawers_world,
):
    world = doors_and_drawers_world
    with symbolic_mode():
    ...
    with rule_mode(query):
        ...
        with next_rule(HasType(revolute_connection.child, Handle)):
            Add(
                views,
                Door(
                    handle=revolute_connection.child,
                    body=revolute_connection.parent,
                    world=world,
                ),
            )
