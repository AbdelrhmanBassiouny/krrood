...
class Rule(SubclassJSONSerializer, ABC):
    ...
    def __post_init__(self):
        ...
        self.node._post_detach = self._post_detach
    ...
    def weight(self, new_weight: RDREdge):
        ...
        if self.node:
            self.node.weight = new_weight.value if new_weight else None
...
class MultiClassTopRule(Rule, HasRefinementRule, HasAlternativeRule):
    ...
    def get_conclusion_as_source_code(
        ...
        statement = (
            f"{parent_indent}    conclusions.update(make_set({conclusion_str}))\n"
        )
        return func, statement
